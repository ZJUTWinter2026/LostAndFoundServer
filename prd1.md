...
| 待审核 | 师生发布的失物/招领信息提交后，尚未经失物招领管理员审核的状态。 |
| 已通过 | 失物/招领信息经管理员审核符合规范，正式发布在系统中的状态。 |
| 未匹配/寻找中 | 通过审核但无人提出认领申请的状态 |
| 已匹配/待认领 | 失物/招领信息被提出认领申请（未完成最终认领）。 |
| 已认领/已归还 | 失主的认领申请被通过，管理员标记后的最终完成状态。 |
| 已驳回 | 管理员审核后认为信息不真实、不完整或违规，拒绝发布的状态（需填写驳回理由）。 |
| 已取消 | 发布者在信息"已通过"但未认领前主动终止发布的状态。 |
| 已归档 | 超过认领时效（初始30天）无人认领的物品，管理员标记的封存状态（需记录处理方式）。 |
...
密码修改页面：
- 包含学号输入框、密码输入框、新密码输入框（密码由6-16位字母（区分大小写）、数字或符号组成）、确认新密码输入框、验证码输入框及验证码图片、确认按钮。
...
忘记密码弹窗：
- 包含学号、身份证号码、新密码、确认密码、验证码输入框，以及验证码图片和"确认"按钮。
- 提示用户"为统一验证系统密码"和"密码由6-16位字母（区分大小写）、数字或符号组成"。
...
- 用户点击"忘记密码"，弹出对应弹窗。
- 点击输入框，出现光标引导用户输入对应信息。
- 输入相关信息，点击"确认"，系统通过学校统一验证后更改密码，用户回到登录页面。
...
**功能描述**：
- 信息筛选功能：用户需选择筛选框中至少一个筛选标准，必须且只能选择该筛选标准中一个内容，再点击"查看"显示信息列表。
...
- 筛选框：位于页面上方，显示物品类型、丢失/拾取校区、时间范围、物品状态
...
- 时间范围：近24小时/3天/7天/30天
- 物品状态：未匹配、已匹配、已认领
- 选择至少一个筛选选项后才可选中"查看"按钮，点击查看：显示物品信息列表。
- 点击"其它类型"：显示物品类型输入框
- 输入文字
- 点击"确认"：按照输入的文字搜索；上传文字数据给系统管理员，同时改变系统管理员-分类标签和各类型占比统计表
- 点击"取消"：隐藏物品类型输入框
...
**后置条件**：
- 提交认领申请：用户"管理发布消息"页的"失物"（对应认领申请中"找回"）或"招领"（对应"归还"）物品状态列表新增一条"已匹配"
...
- 点击"其它类型"：显示物品类型输入框
- 输入文字
- 点击"确认"：该物品信息中"物品类型"是输入的文字；上传数据给系统管理员，同时改变系统管理员-分类标签和各类型占比统计表
- 点击"取消"：隐藏物品类型输入框
...
**功能描述**：
- 物品状态查看与修改：看所有发布的失物或招领记录，并展示物品状态，状态包括："待审核""已通过""已匹配""已认领""已驳回（显示原因）""已取消"。
- 对于"待审核"状态的记录，可修改或删除；对于"已通过"但未完成认领的记录，可补充信息或取消发布。
...
- "失物"或"招领"物品状态列表
- "待审核"的记录：有"修改""删除"按钮
- "已通过"的记录：有"补充说明""取消发布"按钮
- 修改弹窗：物品名称、类型、丢失地点、丢失时间、物品特征、联系人、联系电话（仅对管理员和失主/拾主可见）、是否有悬赏（可选），显示图片（如有），可上传物品相关照片（如有）
- 补充说明弹窗：物品名称、类型、拾取地点、拾取时间、物品特征、联系人及联系电话，显示图片（如有），有上传图标，提交的照片最多提交3张（居左排列）
- 确认删除弹窗

**填充逻辑**
- 物品状态列表：按照"待审核""已通过""已匹配""已认领""已驳回（显示原因）""已取消"从上到下排列，每一种状态按照时间由晚及早地从上到下排列
- "待审核"的记录：有"修改""删除"按钮
- "已通过"的记录：有"补充说明""取消发布"按钮
- 修改弹窗：从上到下依次为物品名称、类型、丢失地点、丢失时间、物品特征、联系人、联系电话（仅对管理员和失主/拾主可见）、是否有悬赏，显示图片（如有），可上传物品相关照片（如有），提交的照片最多提交3张（居左排列）
- 补充说明弹窗：从上到下依次为物品名称、类型选项、拾取地点、拾取时间、物品特征、联系人及联系电话，是否有悬赏，显示图片（如有），有上传图标，提交的照片最多提交3张（居左排列）
...
- 点击"已通过"记录的"补充说明"按钮：显示补充说明弹窗，包括：显示修改弹窗以及物品各个信息的输入框或选项，输入文字或选择，点击上传图标可上传照片。点击"确认"上传信息，点击"取消"隐藏弹窗
...
- 点击"已通过"记录的"补充说明"按钮：点击"确认"上传信息给系统管理员，同时修改列表和改物品详情页内容
...
密码修改页面：
- 学号、原密码、新密码、确认新密码、验证码输入框初始为提示语，输入后更新为输入内容。
...
密码修改交互：
- 点击"忘记密码"选项，进入密码修改页面。
- 输入学号、原密码、新密码、确认新密码和验证码后，点击确认按钮。
...
**用户场景**：失物招领管理员需要查看和审核用户发布过的信息并且查看历史审核记录，通过点击系统主页顶部中间位置"审核发布信息"板块进入该页面，审核并管理相关信息

**功能描述**：
- 审核提交的发布信息：查看、通过或驳回未审核列表；若驳回，须输入理由
- 查看历史审核记录：查看记录列表与详细信息
...
- 次级导航栏："失物"、"招领"、"历史审核记录"，顶部导航栏下方
- "失物"、"招领"待审核列表：次级导航栏下方
- 历史审核记录列表：次级导航栏下方；仅显示本校区，失物招领管理员仅可审核本校区的发布信息
...
- 历史审核记录列表：次级导航栏下方；按照时间从晚到早，从上到下排列
...
**补充说明**：已匹配（有用户认领的发布信息）、未匹配（无人认领的已通过审核的发布信息）、已归档（超过认领时效（初始值30天）无人认领才可判定为"已归档"）
...
- 可直接在文档中修改内容，内容同步到学生/老师界面-发布消息页-"《失物招领内容规范》"链接
...
- "确认"：信息填写栏下方
- 文字提示"创建成功！"：页面中央；初始隐藏，点击"确认"、通过系统验证、备份完成后显示
...
**用户场景**：系统管理员需要备份、导出、清理数据，处理用户投诉与反馈（如信息错误、恶意发布等）。通过点击系统主页顶部中间位置"数据管理"板块进入该页面

**功能描述**：
- 备份导出：备份、导出全校失物招领系统数据；
- 清理数据：清理过期无效数据；
...
全校失物招领信息页：
- "备份"、"导出"、"过期无效数据"：位于次级导航栏下方
- "清理"：位于"备份"、"导出"、"过期无效数据"的下方，初始隐藏，点击"过期无效数据"后显示
